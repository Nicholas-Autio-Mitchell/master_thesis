...
# The Twitter data shall be extracted and cleaned here, ready to be merged with data from comparable files
# The colection of data can then be used together in one model

# Introduction - the idea behind the method -------------------------------

## Sentiment analysis on search terms: "Dow Jones" on Twitter to be used as a regression factor

# Taking the frequency alone of non-directly-related Twitter data (e.g. Obama - from his account and/or from the entire Twitter platform) is rather arbitrary in itself as we do not know what factors affect to number of tweets.
# Here we take Twitter data, the majority of which should be directly related to the dependent variable (i.e. the Dow Jones Index)
# We can therefore quantify the data, at least to begein with, by frequency.
# For example we take daily granularity and measure the frequency of tweets per day related to our target (Dow Jones)
# We inspect these frequencies visually against the market for usefulness and validity before proceeding to a modelling approach
