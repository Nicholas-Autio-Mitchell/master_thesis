...
all_results.glm.cutoff80.frame40 <- foreach(i = 1:length(input_data.one_list),
                                            .combine = append,
...
save(all_results.glm.cutoff80.frame40, file = "all_results.glm.cutoff80.frame40.rda")
...
for(i in 1:5) {                         #i represents the lag value 
    ...
    my_results_par <- foreach(j = 1:length(input_data[[i]]), #j represents the subset
                              ...
                              .packages = c("mboost", "caret", "data.table")) %dopar%
        {
            ...
            to_save <- paste0("glm_results.L", as.character(names(input_data)[[j]])) #desired name
            assign(to_save, tmp_results)        #assign data to desired name
...
look at the sign_accuracy
x <- matrix(NA, nrow = length(glm_results.L3.trad_small$results), ncol = 1)
for(i in 1:length(glm_results.L3.trad_small$results)){x[i] <- glm_results.L3.trad_small$results[[i]][[4]][[5]]}
sum(x)/length(glm_results.L3.trad_small$results)

y <- matrix(NA, nrow = length(glm_results.L3.trad_large$results), ncol = 1)
for(i in 1:length(glm_results.L3.trad_large$results)){y[i] <- glm_results.L3.trad_large$results[[i]][[4]][[5]]}
sum(y)/length(glm_results.L3.trad_large$results)

a <- matrix(NA, nrow = length(glm_results.L3.sent_small$results), ncol = 1)
for(i in 1:length(glm_results.L3.sent_small$results)){a[i] <- glm_results.L3.sent_small$results[[i]][[4]][[5]]}
sum(a)/length(glm_results.L3.sent_small$results)

b <- matrix(NA, nrow = length(glm_results.L3.sent_large$results), ncol = 1)
for(i in 1:length(glm_results.L3.sent_large$results)){b[i] <- glm_results.L3.sent_large$results[[i]][[4]][[5]]}
sum(b)/length(glm_results.L3.sent_large$results)

c <- matrix(NA, nrow = length(glm_results.L3.combined$results), ncol = 1)
for(i in 1:length(glm_results.L3.combined$results)){c[i] <- glm_results.L3.combined$results[[i]][[4]][[5]]}
sum(c)/length(glm_results.L3.combined$results)
