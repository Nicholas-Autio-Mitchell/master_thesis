...
all_results.glm.cutoff70.frame60 <- foreach(i = 1:length(input_data.one_list),
                                            .combine = append,
...
save(all_results.glm.cutoff70.frame60, file = "all_results.glm.cutoff70.frame60.rda")
...
## Quickly check the aggregate results
x <- matrix(NA, nrow = length(glm_results.L4.combined$results), ncol = 1)
for(i in 1:length(glm_results.L4.combined$results)){x[i] <- glm_results.L4.combined$results[[i]][[4]][[5]]}
sum(x)/length(glm_results.L4.combined$results)

y <- matrix(NA, nrow = length(glm_results.L3.combined$results), ncol = 1)
for(i in 1:length(glm_results.L3.combined$results)){y[i] <- glm_results.L3.combined$results[[i]][[4]][[5]]}
sum(y)/length(glm_results.L3.combined$results)

z <- matrix(NA, nrow = length(glm_results.L3.trad_large$results), ncol = 1)
for(i in 1:length(glm_results.L3.trad_large$results)){z[i] <- glm_results.L3.trad_large$results[[i]][[4]][[5]]}
sum(z)/length(glm_results.L3.trad_large$results)

glm_results.L1.combined

a <- matrix(NA, nrow = length(glm_results.L1.combined$results), ncol = 1)
for(i in 1:length(glm_results.L1.combined$results)){a[i] <- glm_results.L1.combined$results[[i]][[4]][[5]]}
sum(a)/length(glm_results.L1.combined$results)
