...
all_results.bin.cutoff80.frame60_growing <- foreach(i = 1:length(input_data.one_list),
                                        .combine = append,
                                        .packages = c("mboost", "caret", "data.table")) %dopar%
    {
        tmp_results <- bin_model(input_data.one_list[[i]], nIter = 1000,
...
save(all_results.bin.cutoff80.frame60_growing, file = paste0(this_folder, "/all_results.bin.cutoff80.frame60_growing.rda"))
