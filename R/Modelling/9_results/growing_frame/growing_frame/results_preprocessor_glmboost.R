...
base_path <- "/Volumes/Mac\ OS\ Drive/Thesis/Source\ Code/R/Modelling/9_results/growing_frame/growing_frame/"
parm_set <- list.files(path = base_path, pattern = "^all_results.")
...
for(i in 1:length(parm_set)) {
    load(file = full_paths[i])
}

results_objects <- sapply(c("binomial", "gaussian"), function(x) NULL)
## Assign the results files to the list
results_objects[[1]] <- all_results.bin.cutoff80.frame60_growing
results_objects[[2]] <- all_results.glm.cutoff80.frame60_growing
...
growing_glm_results <- sapply(names(results_objects$gaussian), function(x) NULL)

for(i in 1:length(results_objects$gaussian)) {
    
    growing_glm_results[[i]] <- results_preprocessor(results_objects$gaussian[[i]])
}

save(growing_glm_results, file = "growing_glm_results.rda")
