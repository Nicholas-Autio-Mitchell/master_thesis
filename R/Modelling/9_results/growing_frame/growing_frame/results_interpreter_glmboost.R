...
## For the growing frame-size data sets: (initial) frame-size = 60, correlationtion threshold = 80 %

## Done for both glm and binomial data, also plotted next to the equivalent for not growing frame

## --------------------------- ##
##  Load the collated results  ##
## --------------------------- ##

## the collated results for growing
#load("/Volumes/Mac OS Drive/Thesis/Source Code/R/Modelling/9_results/glm_boost/glm_collated.results.rda")
load("/Volumes/Mac OS Drive/Thesis/Source Code/R/Modelling/9_results/growing_frame/growing_frame/growing_glm_results.rda")
load("/Volumes/Mac OS Drive/Thesis/Source Code/R/Modelling/9_results/growing_frame/growing_frame/bin_results.collated.rda")

## for comparison, the main results of the same frame-size and cutoff (directly comparable)
load("/Volumes/Mac OS Drive/Thesis/Source Code/R/Modelling/9_results/glm_boost/glm_collated.results.rda") #glm
plain_glm <- glm_collated.results$B_cutoff80_frame60
load("/Volumes/Mac OS Drive/Thesis/Source Code/R/Modelling/9_results/binomial/binomial_collated.results.rda")
plain_bin <- binomial_collated.results$B_cutoff80_frame60


## ======================================================= ##
##  Extract pred accuracies for the growing frame results  ##
## ======================================================= ##
...
glm.growing.pred_acc <- data.table(Lag = 1:5, trad_small = matrix(0, 5, 1), trad_large = matrix(0, 5, 1),
...
for(i in 1:5) {                     #assigns the columns one-yby-one
    ...
    for(j in seq(0, 20, by = 5)) {
        ...
        glm.growing.pred_acc[curr_lag, (i+1)] <- growing_glm_results[[k]][[2]][[2]]
    }
}
...
setnames(glm.growing.pred_acc, old = names(glm.growing.pred_acc), new = c("Lag", "trad_small", "trad_large"," sent_small", " sent_large", "combined"))
melted_glm <- melt(glm.growing.pred_acc, id.vars = "Lag")


glm_growing.plot <- ggplot(data = melted_glm, aes(x = Lag, y = value, colour = variable)) +
    geom_line(size = 1.3) +
    theme_bw() +
    theme(legend.direction = "horizontal", legend.position = "bottom") + #this differs from what is in the thesis
    labs(x = "Number of lags", y = "Predictive accuracy (%)")

glm_growing.plot

save(glm_growing.plot, file = "glm_growing.plot.png")

## Copy and add column for facetting
melted_glm_growing <- copy(melted_glm)
melted_glm_growing$model_type <- "Gaussian"
melted_glm_growing$frame_type <- "Increasing frame"
...
## Container for each data subset (of frame-size and cutoff pairs)
bin.growing.pred_acc <- data.table(Lag = 1:5, trad_small = matrix(0, 5, 1), trad_large = matrix(0, 5, 1),
                            sent_small = matrix(0, 5, 1), sent_large = matrix(0, 5, 1), combined = matrix(0, 5, 1))
...
for(i in 1:5) {                     #assigns the columns one-yby-one
    ...
    for(j in seq(0, 20, by = 5)) {
        ...
        curr_lag <- (j/5)+1
        #print(paste0(k,  "  ", curr_lag, "  ", i+1))
        bin.growing.pred_acc[curr_lag, (i+1)] <- growing_bin_results[[k]][[1]][[2]]
    }
}
...
setnames(bin.growing.pred_acc, old = names(bin.growing.pred_acc), new = c("Lag", "trad_small", "trad_large"," sent_small", " sent_large", "combined"))
melted_bin <- melt(bin.growing.pred_acc, id.vars = "Lag")


bin_growing.plot <- ggplot(data = melted_bin, aes(x = Lag, y = value, colour = variable)) +
    geom_line(size = 1.3) +
    theme_bw() +
    theme(legend.direction = "horizontal", legend.position = "bottom") + #this differs from what is in the thesis
    labs(x = "Number of lags", y = "Predictive accuracy (%)")
...
save(bin_growing.plot, file = "bin_growing.plot.png")


## Copy data and add column for facetting
melted_bin_growing <- copy(melted_bin)
melted_bin_growing$model_type <- "Binomial"
melted_bin_growing$frame_type <- "Increasing frame"


###################### ======================================= ######################
######################  Extract pred accs for plain data sets  ######################
###################### ======================================= ######################
...
glm.plain.pred_acc <- data.table(Lag = 1:5, trad_small = matrix(0, 5, 1), trad_large = matrix(0, 5, 1),
                            sent_small = matrix(0, 5, 1), sent_large = matrix(0, 5, 1), combined = matrix(0, 5, 1))
...
for(i in 1:5) {                     #assigns the columns one-yby-one
    ...
    for(j in seq(0, 20, by = 5)) {
        ...
        curr_lag <- (j/5)+1
        #print(paste0(k,  "  ", curr_lag, "  ", i+1))
        glm.plain.pred_acc[curr_lag, (i+1)] <- plain_glm[[k]][[2]][[2]]
    }
}

setnames(glm.plain.pred_acc, old = names(glm.plain.pred_acc), new = c("Lag", "trad_small", "trad_large"," sent_small", " sent_large", "combined"))
melted_glm_plain <- melt(glm.plain.pred_acc, id.vars = "Lag")

## Add column for facetting
melted_glm_plain$model_type <- "Gaussian"
melted_glm_plain$frame_type <- "60-day frame"

## ---------- ##
##  Binomial  ##
## ---------- ##

bin.plain.pred_acc <- data.table(Lag = 1:5, trad_small = matrix(0, 5, 1), trad_large = matrix(0, 5, 1),
                            sent_small = matrix(0, 5, 1), sent_large = matrix(0, 5, 1), combined = matrix(0, 5, 1))
...
for(i in 1:5) {                     #assigns the columns one-yby-one
    ...
    for(j in seq(0, 20, by = 5)) {
        ...
        curr_lag <- (j/5)+1
        #print(paste0(k,  "  ", curr_lag, "  ", i+1))
        bin.plain.pred_acc[curr_lag, (i+1)] <- plain_bin[[k]][[1]][[2]]
    }
}

setnames(bin.plain.pred_acc, old = names(bin.plain.pred_acc), new = c("Lag", "trad_small", "trad_large"," sent_small", " sent_large", "combined"))
melted_bin_plain <- melt(bin.plain.pred_acc, id.vars = "Lag")

## Add column for facetting
melted_bin_plain$model_type <- "Binomial"
melted_bin_plain$frame_type <- "60-day frame"

###################### ================================ ######################
######################  Combine all results to compare  ######################
###################### ================================ ######################

## Need to make the percentages all on the same scale (bin results are as decimals)
melted_bin_plain$value <- 100 * melted_bin_plain$value
melted_bin_growing$value <- 100 * melted_bin_growing$value

## Combine all data sets to get on same plot with facets
plain_growing <- rbind(melted_glm_plain, melted_glm_growing, melted_bin_plain, melted_bin_growing)

## --------------------------- ##
##  Plot all results together  ## --> Final plot for thesis
## --------------------------- ## 

setnames(plain_growing, old = "variable", new = "Subset  ")

plain_grow <- ggplot(data = plain_growing, aes(x = Lag, y = value, colour = `Subset  `)) +
    facet_grid(facets = frame_type  ~ model_type) +
    geom_line(size = 1.3) +
    theme_bw(base_size = 20) +
    theme(legend.direction = "horizontal", legend.position = "bottom") + #this differs from what is in the thesis
    scale_fill_continuous(guide = guide_legend(title = "Subset")) +
...
###################### ========================================== ######################
######################  From old copy of code for larger subsets  ######################
###################### ========================================== ######################
## -------------------------------------------- ##
