...
all_results.bin.no_cutoff.frame100 <- foreach(i = 1:length(input_data.one_list),
                                        .combine = append,
                                        .packages = c("mboost", "caret", "data.table")) %dopar%
    {
        tmp_results <- bin_model(input_data.one_list[[i]], nIter = 2000,
                                   shrinkage = 0.05, frame_size = 100)
...
save(all_results.bin.no_cutoff.frame100, file = paste0(this_folder, "/all_results.bin.no_cutoff.frame100.rda"))
