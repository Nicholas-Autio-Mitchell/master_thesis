...
## ------------- ##
##  Information  ## --> called 'corr_pruner.R' without binaomial etc for a reason!
## ------------- ##

#' This file should be unchanged for all modelling variations. It is to work on the base data subsets,
#' where DOW.DP is a numeric value. Only after correlation reduction has been implemented should further
#' data transformation take place. E.g., creating a binary response for the DOW.DP (into DOW.bin) happens
#' AFTER this script has been used.
...
## It then re-calculates cumulative pairwise correlation for each variable, given that one has just been removed.
## this process is repeated until no variables show a pairwise correlation higher that the user specified cutoff.
...
corr_pruner <- function(input, cutoff = .85,                           #choose correlation cutoff level
                 plot.cutoff = FALSE, low_corr = .65, high_corr = .99, #plot with limits low and high
                 print.removed = FALSE, verbose = FALSE, #show names of variables removed
                 s_name) {         #name of current subset in function (no default)
    ...
    if(length(to_remove) == 0) {
        message(paste0(s_name,
                       " - Number of covariates removed with supplied correlation cutoff level: 0"))
        ...
        message(paste0(s_name,
                       " - Number of covariates removed with supplied correlation cutoff level: ",
