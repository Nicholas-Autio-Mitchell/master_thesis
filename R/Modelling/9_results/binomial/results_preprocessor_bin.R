...
## Main group split into parameter classes, cutoff and frame sizes - 20 variations at present
bin_results.closing_prices <- sapply(names(results_objects), function(x) NULL)
## Each set of parameter classes has five model subsets and five lags, so 25 names in total for each parameter set
subsets <- sapply(names(results_objects$A_cutoff30_frame40), function(x) NULL) #use any parameter set to get model names
## Apply a subset to each parameter set
...
for(i in 1:length(results_objects)) {
    for(j in 1:length(results_objects[[i]])) {
        bin_results.closing_prices[[i]][[j]] <- results_preprocessor(results_object = results_objects[[i]][[j]])
    }
}
...
saveRDS(object = bin_results.closing_prices,
        file = "/Volumes/Mac OS Drive/Thesis/Source Code/R/Modelling/9_results/binomial/bin_results.closing_prices.rds")
...
## =========== ##
## ======================================================== ##
##  Combine the results that didn't make it in one session  ##
## ======================================================== ##

## Create necessary paths to load all data
base_path <- "/Volumes/Mac\ OS\ Drive/Thesis/Source\ Code/R/Modelling/9_results/binomial/"
parm_set <- c("C_cutoff80_frame80", "D_cutoff80_frame100")
full_paths <- paste0(base_path, parm_set, "/results/")

to_load1 <- matrix(NA, 25, 1)
to_load2 <- matrix(NA, 25, 1)

## The file names are actually identical, but lets keep them in two lists
## The files that we want to load for the first set
to_load1 <- c(list.files(full_paths[1], "^bin_results.Lag."))
## The files that we want to load for the second set
to_load2 <- c(list.files(full_paths[2], "^bin_results.Lag."))


## Create the final list to assign all results to
all_results.bin.cutoff80.frame80 <- sapply(to_load1, function(x) NULL)
all_results.bin.cutoff80.frame100 <- sapply(to_load2, function(x) NULL)

## Load all data for first list
for(i in 1:length(to_load1)) {
    load(file = paste0(full_paths[1], to_load1[i]))
}
## Assign the results files to the list --> ~1Gb
for(i in 1:length(to_load1)) {
    all_results.bin.cutoff80.frame80[[i]] <- get(substr(to_load1[i], start = 1, stop = (nchar(to_load1[i])-4)))
}

save(all_results.bin.cutoff80.frame80, file = "all_results.bin.cutoff80.frame80.rda")


## Load all data for first list
for(i in 1:length(to_load2)) {
    load(file = paste0(full_paths[2], to_load2[i]))
}
## Assign the results files to the list --> ~1Gb
for(i in 1:length(to_load2)) {
    all_results.bin.cutoff80.frame100[[i]] <- get(substr(to_load2[i], start = 1, stop = (nchar(to_load2[i])-4)))
}

save(all_results.bin.cutoff80.frame100, file = "all_results.bin.cutoff80.frame100.rda")
