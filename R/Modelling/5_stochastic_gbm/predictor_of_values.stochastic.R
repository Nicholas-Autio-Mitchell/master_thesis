...
source(paste0(this_folder, "/results_template.stochastic.R"))
source(paste0(this_folder, "/ultra_modeller.stochastic.R"))
...
x <- input_data.one_list$Lag.1.trad_small
...
all_results.stoch.cutoff80.frame60 <- foreach(i = 1:length(input_data.one_list),
                                        .combine = append,
                                        .packages = c("caret", "data.table", "gbm")) %do%
    {
        tmp_results <- stoch_model(input_data.one_list[[i]], nIter = 5000,
                                   learnRate = 0.01, frame_size = 60)
    
        to_save <- paste0("stoch_results.", as.character(names(input_data.one_list)[[i]])) #desired name
...
save(all_results.stoch.cutoff80.frame60, file = paste0(this_folder, "/all_results.stoch.cutoff80.frame60.rda"))
b
