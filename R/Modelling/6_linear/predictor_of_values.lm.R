...
source(paste0(this_folder, "/results_template.lm.R"))
source(paste0(this_folder, "/error_functions.lm.R"))
source(paste0(this_folder, "/ultra_modeller.lm.R"))
...
all_results.lm.cutoff90.frame60 <- foreach(i = 1:length(input_data.one_list),
                                        .combine = append,
                                        .packages = c("caret", "data.table")) %dopar%
    {
        tmp_results <- lm_model(input_data.one_list[[i]], nIter = 2000,
                                   shrinkage = 0.05, frame_size = 60)
    
        to_save <- paste0("lm_results.", as.character(names(input_data.one_list)[[i]])) #desired name
...
save(all_results.lm.cutoff90.frame60, file = paste0(this_folder, "/all_results.lm.cutoff90.frame60.rda"))
