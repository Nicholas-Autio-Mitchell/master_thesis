...
clu <- makeCluster(4)
registerDoMC(clu)
clusterCall(clu, function(x) .libPaths(x), .libPaths())
registerDoMC(4)
## Windows
#install.packages("doParallel")
library(doParallel)
                      ...
                      frame_size, look_ahead, nIter, shrinkage) # taken from inputs to my_CV
{
    ...
    step_results <- sapply(c("mstop", "actual", "forecast", "errors", "coef_names"), function(x) NULL)
...
## ======================================================== ##
##  Loop through each lag value to create the four subsets  ##
## ======================================================== ##
           ...
           updates = TRUE)                           #print loop number and time at intervals
{
    ...
    cv_sets <- createTimeSlices(data_in$log_ret_DJI, initialWindow= frame_size, #first argument must be one vector, providing the timeseries' length
    ...
    num_steps <- length(cv_sets$train)
        ...
        ##for(i in 1:1) {
    {
        ...
        if (RNG.seed == TRUE) {
            set.seed.alpha(paste0("Nicholas_Mitchell_", i)) #i = the cv_number
        } else {
        ...
        ## Assign the names of the coefficients
...
for(j in 1:length(input_data)) {
    ...
    ## Running 
    tmp_results <- my_CV(data_chunk, nIter = 2000, shrinkage = 0.05)
    ...
    saveRDS(list = to_save, file = file_name) #save data with desired names
...
for(k in 1:5) {
    ...
    for(j in 1:length(input_data[[k]])) {
        ...
        ## Running 
        tmp_results <- my_CV(data_chunk, nIter = 1000, shrinkage = 0.05)
        
        to_save <- paste0("glm_results_", as.character(names(input_data)[[j]])) #desired name
        assign(to_save, tmp_results)        #assign data to desired name
        file_name <- paste0("glm_results_lag_", as.character(k), "_",
                            as.character(names(input_data[[k]])[[j]]), ".rds") #create desired file name
        saveRDS(to_save, file = file_name) #save data with desired names
...
x <- readRDS("/Volumes/Mac OS Drive/Thesis/Source Code/R/Modelling/glm_results_lag_4_Mix.rds")
###################### ====================== ######################
...
#' that all data is chunker

used <- function(data_in, frame_size = 40, num_chunks = 10) {
...
for(i in 1:num_batches){
    ...
    ## Running 
    tmp_results <- my_CV(data_chunk)    #use defaults
