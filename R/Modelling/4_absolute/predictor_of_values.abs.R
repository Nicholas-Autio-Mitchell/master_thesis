...
all_results.abs.cutoff90.frame100 <- foreach(i = c(9,15), #length(input_data.one_list),
                                        .combine = append,
                                        .packages = c("mboost", "caret", "data.table")) %dopar%
    {
        tmp_results <- abs_model(input_data.one_list[[i]], nIter = 2000,
...
save(all_results.abs.cutoff90.frame100, file = paste0(this_folder, "/all_results.abs.cutoff90.frame100.rda"))
